...
export const getSpecialGemsList = () => db.collection('specialStonesIds')
  .get()
  .then(collection => collection.docs.map(doc => doc.get('stoneId')));
...
export const getNewReferralPoints = (refPointsContract, userId) => {

    return refPointsContract &&
      refPointsContract.methods && refPointsContract.methods.balanceOf(userId)
        .call()
        .then(referralPoints => referralPoints)
        .catch(error => setError(error));
}
